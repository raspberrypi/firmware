...
//Min. no. of bytes in Rx message (excluding header) before it is classified as long 
//(which results in VC_CEC_RX_LONG callback instead of VC_CEC_RX)
//Cannot be more than 7
#define CEC_LONG_RX_MSG_SIZE     7

#if CEC_LONG_RX_MSG_SIZE > 7
#error "Threshold for long CEC message cannot be more than 7"
#endif
 ...
 * VC_CEC_RX_LONG          same as VC_CEC_RX                  Message is too long to fit in VC_CEC_RX
 *                                                            Host must use CEC service to fetch the message.
 */
